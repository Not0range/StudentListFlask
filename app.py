...
import requests
import jwt
secret = 'TG9yZW0gaXBzdW0gZG9sb3Igc2l0IGFtZXQsIGNvbnNlY3RldHVyIGFkaXBpc2NpbmcgZWxpdC4='
app = Flask(__name__)
studentList = []
...
def get_list():
    res = requests.post('http://sql:4200/_sql', 
...
def add_student():
    if requests.post('http://sql:4200/_sql',
    ...
    + request.cookies['id'] + '\''}).json()['rowcount'] > 0:
        ...
        if 'id' in obj and 'fio' in obj and 'course' in obj and 'spec' in obj and 'number' in obj:
            requests.post('http://sql:4200/_sql',
...
def check_login():
    if 'id' in request.cookies:
        obj = requests.post('http://sql:4200/_sql',
